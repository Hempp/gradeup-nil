...
export async function generateNILActivityReport(
  ...
  try {
    ...
    // Transform deals to report rows
...
export async function generateBrandActivityReport(
  ...
  try {
    ...
    // Get deals for these brands
    let dealsQuery = supabase
      .from('deals')
      .select('id, brand_id, athlete_id, compensation_amount, status')
    ...
    const dealsByBrand: Record<string, typeof deals> = {};
    for (const deal of deals || []) {
    ...
    for (const brand of brands || []) {
      ...
      // Count unique athlete partnerships
...
export async function generateComplianceViolationsReport(
  ...
  try {
    // Get all deals and identify potential violations
    let query = supabase
      .from('deals')
      .select(`
        id,
    ...
    // Identify violations and at-risk deals
 ...
 * Note: Actual PDF generation should be done on the client or via a specialized service
 */
...
function determineComplianceStatus(deal: {
  ...
  // Check for high-value deals that may need review
  if (deal.compensation_amount > 100000) {
    return 'pending_review';
  }

  // Check if deal has compliance issues based on status
...
function checkForViolations(deal: {
  ...
  // Check for high-value deals exceeding thresholds
  if (deal.compensation_amount > 100000) {
    return {
