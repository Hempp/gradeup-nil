...
(function() {
    ...
    // Demo athlete data - loaded from external file for cleaner organization
    let athletesData = [];

    // Load demo data with proper error handling
    fetch('./src/data/athletes.json')
      .then(response => {
      ...
      .catch(error => {
        console.error('[GradeUp] Could not load athletes data:', error);
        // Show fallback UI using safe DOM methods
        const grid = document.getElementById('athletesGrid');
        if (grid) {
          const errorMsg = document.createElement('p');
          errorMsg.style.cssText = 'color: var(--gray-400); text-align: center; padding: 2rem;';
          errorMsg.textContent = 'Unable to load athletes. Please refresh the page.';
